...
const PriosList = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://192.168.1.9:5000/users/users/alls?role=2')
  ...
  const handleConfirmEdit = async () => {
    try {
      ...
      await upgradeUser(selectedUser, 'down');
  ...
  const columns = [
    ...
    {
      field: 'Changer Role au Joueur',
      headerName: 'Changer Role au Joueur',
      width: 200,
      renderCell: (params) => (
  ...
  return (
    <div className="users-list-container">
      <h2 className="users-list-heading"> <FontAwesomeIcon icon={faList} />Prios List</h2>
      <DataGrid rows={users} columns={columns} pageSize={8} style={{ textDecoration: "none",fontSize:'18px',marginTop:'-15px' }}/>
